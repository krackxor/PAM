...
def upload_mb_data():
    ...
    try:
        ...
        if 'TGL_BAYAR' in df.columns:
            ...
            # 1. Coba parse sebagai tanggal Pandas (Default, coba infer format)
            df['TGL_BAYAR_OBJ'] = pd.to_datetime(
                 df['TGL_BAYAR'].astype(str).str.strip(), 
                 errors='coerce' 
            )
            
            # 2. Jika masih ada NaN, coba format spesifik (e.g., DD-MM-YYYY)
            ...
            # 3. Coba parse dari format float/int (Excel serial date number)
            # Menggunakan origin '1899-12-30' sebagai standar untuk konversi Excel date number
            numeric_dates = pd.to_numeric(df['TGL_BAYAR'].replace({'N/A': float('nan')}), errors='coerce')
            
            # 4. Mengisi nilai NaN dari parsing string dengan hasil konversi Excel serial date
...
def critical_alerts_api():
    ...
    try:
        ...
        pipeline_critical_debt = [
            # FIX: Hanya hitung tunggakan yang nominalnya > 0
            {'$match': {'JUMLAH': {'$gt': 0}}},
            {'$group': {
                '_id': '$NOMEN',
