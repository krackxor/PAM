...
def collection_riwayat_view():
    return render_template('analysis_report_template.html', 
                            title="Riwayat Koleksi Bulanan (MoM)",
...
def top_debtors_report_api():
    ...
    try:
        pipeline = [
            ...
            {'$project': {
                'NOMEN': 1, 'RAYON': 1, 'BULAN_TAGIHAN': 1,
...
def mom_comparison_report_api():
    ...
    try:
        ...
        if not M_month:
            return jsonify({'status': 'error', 'message': 'Tidak ada data Master Cetak (MC) ditemukan.', 'data': []}), 404

        M_minus_1_month = _get_previous_month_year(M_month)
        
        if not M_minus_1_month:
            return jsonify({'status': 'error', 'message': f'Tidak dapat menghitung bulan sebelum {M_month}.', 'data': []}), 404
        ...
        pipeline_mc = [
            ...
            {'$project': {
                ...
                'AB_SUNTER': {'$cond': [ 
                    {'$in': ['$RAYON', ['34', '35']]}, 'AB SUNTER', 'LUAR AB SUNTER'
                ]}, 
            ...
            {'$group': {
                '_id': {
                    'AB_SUNTER': '$AB_SUNTER', 'RAYON': '$RAYON', 'PCEZ': '$PCEZ',
                    'TARIF': '$TARIF', 'MERK': '$MERK', 'CYCLE': '$BOOKWALK', 'BULAN_TAGIHAN': '$BULAN_TAGIHAN'
                },
            ...
            {'$group': {
                '_id': {
                    'AB_SUNTER': '$AB_SUNTER', 'RAYON': '$RAYON', 'PCEZ': '$PCEZ',
                    'TARIF': '$TARIF', 'MERK': '$MERK', 'CYCLE': '$CYCLE'
                },
...
def dod_comparison_report_api():
    ...
    try:
        ...
        pipeline_mb = [
            {'$match': {
                'TGL_BAYAR': {'$in': [D_date, D_minus_1_date]},
                'BILL_REASON': 'BIAYA PEMAKAIAN AIR'
            }},
            {'$lookup': {
               'from': 'CustomerData', 'localField': 'NOMEN', 'foreignField': 'NOMEN', 'as': 'customer_info'
            }},
            {'$unwind': {'path': '$customer_info', 'preserveNullAndEmptyArrays': True}},
            {'$project': {
            ...
            {'$group': {
                '_id': {
                    'AB_SUNTER': '$AB_SUNTER', 'RAYON': '$RAYON', 'PCEZ': '$PCEZ', 
                    'TARIF': '$TARIF', 'MERK': '$MERK', 'CYCLE': '$CYCLE', 
                    'LKS_BAYAR': '$LKS_BAYAR', 'TGL_BAYAR': '$TGL_BAYAR'
                },
            ...
            {'$project': {
                ...
                'TGL_BAYAR': '$_id.TGL_BAYAR',
                'TotalKoleksi': {'$round': ['$TotalKoleksi', 0]},
                'JumlahTransaksi': '$JumlahTransaksi',
            }},
            {'$group': {
                '_id': {
                    'AB_SUNTER': '$AB_SUNTER', 'RAYON': '$RAYON', 'PCEZ': '$PCEZ', 
                    'TARIF': '$TARIF', 'MERK': '$MERK', 'CYCLE': '$CYCLE', 'LKS_BAYAR': '$LKS_BAYAR'
                },
...
# [Tambahkan API Distribution lainnya di sini (rayon, pcez, tarif, meter)]
# ... (API lainnya, disarankan dipindahkan ke file utilitas atau routes_collection)
# Untuk saat ini, kita hanya akan memindahkannya di sini agar tetap modular.
...
def rayon_distribution_report():
    ...
    for item in results: item['chart_label'] = item.get("RAYON", "N/A"); item['chart_data_nomen'] = item['total_nomen']; item['chart_data_piutang'] = round(item['total_piutang'], 2)
...
def pcez_distribution_report():
    ...
    results, latest_month = _get_distribution_report(group_fields=group_fields, collection_mc=collection_mc)
    schema = _generate_distribution_schema(group_fields)
    for item in results: item['chart_label'] = item.get("PCEZ", "N/A"); item['chart_data_nomen'] = item['total_nomen']; item['chart_data_piutang'] = round(item['total_piutang'], 2)
...
def rayon_tarif_distribution_report():
    ...
    results, latest_month = _get_distribution_report(group_fields=group_fields, collection_mc=collection_mc)
    schema = _generate_distribution_schema(group_fields)
    for item in results: label = f"{item.get('RAYON', 'N/A')} - {item.get('TARIF', 'N/A')}"; item['chart_label'] = label; item['chart_data_nomen'] = item['total_nomen']; item['chart_data_piutang'] = round(item['total_piutang'], 2)
...
def rayon_meter_distribution_report():
    ...
    results, latest_month = _get_distribution_report(group_fields=group_fields, collection_mc=collection_mc)
    schema = _generate_distribution_schema(group_fields)
    for item in results: label = f"{item.get('RAYON', 'N/A')} - {item.get('JENIS_METER', 'N/A')}"; item['chart_label'] = label; item['chart_data_nomen'] = item['total_nomen']; item['chart_data_piutang'] = round(item['total_piutang'], 2)
