...
# ==========================================
# PAM DSS - MAGIC INSTALLER (FULL STACK)
# ==========================================

echo "🛑 [1/7] BERSIH-BERSIH SYSTEM..."
...
echo "📦 [2/7] INSTALL DATABASE (MONGODB)..."
if ! systemctl is-active --quiet mongod; then
    ...
    echo "✅ MongoDB Installed & Started."
else
    echo "✅ MongoDB sudah aktif."
fi

echo "🔍 [3/7] UPDATE NODE.JS KE V20..."
current_ver=$(node -v 2>/dev/null | cut -d'.' -f1 | sed 's/v//')
if [ "$current_ver" != "20" ] && [ "$current_ver" != "21" ]; then
...
echo "📝 [4/7] MENULIS ULANG FILE SISTEM..."
...
# 2. UTILS.PY (SMART MAPPING & DB SAVE)
cat > utils.py << 'EOF'
import os
import pandas as pd
...
load_dotenv()
# Gunakan MongoDB Local jika tidak ada URI di env
...
def save_to_db(data, type_str):
    ...
    try:
        ...
        elif 'ARREARS' in type_str or 'ARDEBT' in type_str: coll_name = 'arrears'
        ...
        # Bersihkan data lama agar tidak duplikat (Opsional, tergantung kebutuhan)
        # db[coll_name].delete_many({}) 
        
        # Simpan
        db[coll_name].insert_many(data, ordered=False)
        print(f"✅ Saved {len(data)} records to {coll_name}")
    except Exception as e: print(f"⚠️ Save DB Error: {e}")
...
def analyze_meter_anomalies(df_records):
    ...
    if db is not None:
        ...
        try: db.meter_history.insert_many(hist_data, ordered=False)
        except: pass
    ...
    for row in df_records:
        ...
        if status:
            anomalies.append({
                'nomen': str(row.get('NOMEN')),
                'name': row.get('NAMA',''),
                'usage': usage,
                'status': status,
                'details': {'anomaly_reason': ', '.join(status), 'skip_desc': skip}
            })
    return anomalies
...
def get_summarized_report(target, dimension, rayon_filter=None):
    ...
    # KARENA SMART MAPPING SUDAH MENYAMAKAN KEY JADI 'NOMINAL'
    val_field = '$NOMINAL' 
...
def upload():
    ...
    try:
        ...
        # SAVE TO DB AGAR SUMMARY MUNCUL
...
def hist():
    ...
    d = get_usage_history(request.args.get('filter_by'), request.args.get('value')) if t == 'usage' else [] # Simplified payment history
    return jsonify({"status":"success", "data": d})
...
echo "🎨 [5/7] GENERATING FRONTEND UI..."
